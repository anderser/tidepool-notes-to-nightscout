...
class NightscoutClient {
  ...
  async addTreatment(nsTreatment) {
    return await this.invokeNSEndpoint('/api/v1/treatments', 'post', nsTreatment);
  }

  async updateTreatment(nsTreatment) {
    return await this.invokeNSEndpoint('/api/v1/treatments', 'put', nsTreatment);
  }
  ...
  async upsertTreatment(tidepoolNote) {
    ...
    if (treatments.length > 0) {
      ...
      if (exsistingTreatment.notes !== nsTreatment.notes) {
        ...
        console.log(`Treatment ${findTimestamp} already exists and is not changed`);
      }
    ...
    return null// await this.getNSEndpoint('/api/v1/treatments.json');
  }
}

/*
Get treatment by created_at

https://nightscout-l.herokuapp.com/api/v1/treatments.json?find[created_at][$eq]=2019-09-12T19:58:35.905Z
